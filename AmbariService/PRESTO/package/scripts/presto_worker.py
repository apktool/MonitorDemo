...
class Worker(Script):
    def install(self, env):
        from params import presto_worker_path
        presto_etc_path = presto_worker_path + '/etc'
        ...
        Directory([presto_worker_path, presto_etc_path, presto_catalog_path], create_parents=True, owner='root', group='root')
        Execute('tar xf {0} -C {1} --strip-components 1'.format('/tmp/presto-server.tar.gz', presto_worker_path))
        Link('/usr/hdp/current/presto-worker', to=presto_worker_path)
    ...
    def start(self, env):
        import params
        self.configure(self)
        launcher = params.presto_worker_path + '/bin/launcher'
    ...
    def configure(self, env):
        ...
        presto_etc_path = params.presto_worker_path + '/etc'
        presto_catalog_path =presto_etc_path + '/catalog'
